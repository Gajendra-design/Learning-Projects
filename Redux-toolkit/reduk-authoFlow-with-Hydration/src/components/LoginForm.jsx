...
const LoginForm = () => {
  const { register, errors, handelLogin, handleSubmit,setShowPassword,showPassword } = useAuth();
  ...
  return (
    <>
      ...
      <div className="relative flex items-center justify-center">
        ...
        <span className="bg-white px-3 text-xs font-medium text-slate-400 uppercase tracking-wider relative">Or continue with</span>
      </div>
      ...
      <form onSubmit={handleSubmit(handelLogin)} className="space-y-5">
        ...
        <div className="flex items-center">
          ...
          />
          <label htmlFor="remember" className="ml-2 text-sm text-slate-600 cursor-pointer">Remember for 30 days</label>
        </div>

        {/* Submit Button */}
        <button 
          type="submit" 
          className="w-full py-3 px-4 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-xl shadow-lg shadow-indigo-500/25 transition-all focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
        >
          Sign in
        </button>
      </form>
      ...
      <p className="text-center text-sm text-slate-500">
        ...
        <Link to="/register" className="font-semibold text-indigo-600 hover:text-indigo-500">Sign up for free</Link>
      </p>
    </>
  )
}
